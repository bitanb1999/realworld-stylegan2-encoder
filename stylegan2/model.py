...
class EqualConv2d(nn.Module):
    def __init__(
        ...
        # )
        self.weight = nn.Parameter(
            torch.zeros(out_channel, in_channel, kernel_size, kernel_size)
        )
...
class ModulatedConv2d(nn.Module):
    def __init__(
        ...
        # )
        self.weight = nn.Parameter(
            torch.zeros(1, out_channel, in_channel, kernel_size, kernel_size)
        )
...
class NoiseInjection(nn.Module):
    ...
    def forward(self, image, noise=None):
        if noise is None:
            ...
            noise = torch.zeros((batch, 1, height, width)).to(image.device)
...
class Generator(nn.Module):
    ...
    def make_noise(self):
        ...
        for i in range(3, self.log_size + 1):
            noises.extend(
                torch.zeros(1, 1, 2**i, 2**i, device=device) for _ in range(2)
            )
        return noises
